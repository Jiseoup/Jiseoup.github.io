...
만약 ***`Please configure the PostgreSQL Binary Path in the Preferences dialog.`*** 라는 에러 문구가 나타난다면 [[pgAdmin] PostgreSQL Binary Path 설정하기](/posts/pgadmin-binary-path/) 포스트를 참고해주세요.  
